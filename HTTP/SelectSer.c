...
Server: nginx\r\n\
Date: Sat, 23 Jun 2018 03:37:15 GMT\r\n\
Content-Type: text/html\r\n\
Last-Modified: Thu, 07 Jun 2018 16:04:02 GMT\r\n\
...
Content-Encoding: gzip\r\n\r\n";
                ...
                int n = send(conn,Http,strlen(Http),0);
                if(n < 0)
                    perror("send http HttpRespos");
                else
                    printf("send http head:%lu--- %d byte\n",strlen(Http),n);
                
                int fd = open("index.html",O_EXCL,0777);
                if(fd < 0)
                    perror("open file failed");
                n = sendfile(conn,fd,NULL,1624);
                if(n < 0)
                    perror("sendfile");
                }
            }             
